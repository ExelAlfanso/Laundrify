...
public class LayananForm extends JFrame {
    ...
    private int editingRow = -1;   // -1 = insert, >=0 = update
    ...
    private void initComponents() {
        ...
        for (int i = 0; i < labels.length; i++) {
            gc.gridx = 0; gc.gridy = i; formPanel.add(new JLabel(labels[i] + ":"), gc);
            gc.gridx = 1; formPanel.add(fields[i], gc);
        }

        btnSimpan = new JButton("Simpan / Perbarui");
        btnClear  = new JButton("Bersihkan");
        btnHapus  = new JButton("Hapus");
        JPanel btnPanel = new JPanel(); btnPanel.add(btnSimpan); btnPanel.add(btnClear); btnPanel.add(btnHapus);
        gc.gridx = 0; gc.gridy = labels.length; gc.gridwidth = 2; formPanel.add(btnPanel, gc);

        String[] cols = {"Nama", "Satuan", "Harga", "Durasi", "ID"};
        tableModel = new DefaultTableModel(cols, 0){ @Override public boolean isCellEditable(int r,int c){ return false;} };
        table = new JTable(tableModel);
        table.removeColumn(table.getColumnModel().getColumn(4)); // sembunyikan ID
    ...
    private void loadData(){
        ...
        try (Connection con = Koneksi.getConnection(); Statement st = con.createStatement(); ResultSet rs = st.executeQuery("SELECT id_layanan,nama_layanan,satuan,harga,durasi_layanan FROM Layanan")){
            while(rs.next()){
                tableModel.addRow(new Object[]{rs.getString(2),rs.getString(3),rs.getDouble(4),rs.getInt(5),rs.getInt(1)});
            }
        }catch(Exception ex){ JOptionPane.showMessageDialog(this,"Gagal load:\n"+ex.getMessage()); }
    }
    ...
    private void saveOrUpdate(){
        ...
        double harga; int durasi;
        try{ harga=Double.parseDouble(txtHarga.getText()); }catch(Exception ex){ JOptionPane.showMessageDialog(this,"Harga tidak valid"); return; }
        try{ durasi=txtDurasi.getText().isBlank()?0:Integer.parseInt(txtDurasi.getText()); }catch(Exception ex){ JOptionPane.showMessageDialog(this,"Durasi tidak valid"); return; }
        ...
        try(Connection con = Koneksi.getConnection()){
            if(editingRow==-1){ // insert
                ...
                try(PreparedStatement ps=con.prepareStatement(sql)){
                    ps.setString(1,nama); ps.setString(2,satuan); ps.setDouble(3,harga); ps.setInt(4,durasi);
                ...
                try(PreparedStatement ps=con.prepareStatement(sql)){
                    ps.setString(1,nama); ps.setString(2,satuan); ps.setDouble(3,harga); ps.setInt(4,durasi); ps.setInt(5,id);
                    ps.executeUpdate();
                    tableModel.setValueAt(nama, editingRow,0); tableModel.setValueAt(satuan,editingRow,1);
                    tableModel.setValueAt(harga,editingRow,2); tableModel.setValueAt(durasi,editingRow,3);
                }
            }
            clearForm();
    ...
    private void deleteSelected(){
        int row=table.getSelectedRow(); if(row==-1){ JOptionPane.showMessageDialog(this,"Pilih data!"); return; }
    ...
    private void fillForm(){
        int row=table.getSelectedRow(); if(row==-1) return; editingRow=row;
    ...
    private void clearForm(){ txtNama.setText(""); txtSatuan.setText(""); txtHarga.setText(""); txtDurasi.setText(""); table.clearSelection(); editingRow=-1; btnSimpan.setText("Simpan"); }

    public static void main(String[] args){ SwingUtilities.invokeLater(() -> new LayananForm().setVisible(true)); }
}